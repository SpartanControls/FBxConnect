        ...
        while (true) {
            ...
            switch(line)
            {
                case("quit"):
                    return;
                case("crob"):
                    ControlRelayOutputBlock crob = new ControlRelayOutputBlock(ControlCode.LATCH_ON, (short) 10, 100, 100, CommandStatus.SUCCESS);
                    ...
                    System.out.println(crob.status);
                    break;
                case("poll"):
                    CommandHeaders header = CommandHeader.fromCROB(
                            Arrays.asList(
                                    new IndexedValue<>(new ControlRelayOutputBlock(ControlCode.LATCH_ON, (short) 1, 100, 100, CommandStatus.SUCCESS), 0),
                                    new IndexedValue<>(new ControlRelayOutputBlock(ControlCode.LATCH_OFF, (short) 1, 100, 100, CommandStatus.BLOCKED), 1)
                            )
                    );
                    master.selectAndOperate(header).thenAccept(
                            //asynchronously print the result of the command operation
                            (CommandTaskResult result) -> System.out.println(result)
                    );
